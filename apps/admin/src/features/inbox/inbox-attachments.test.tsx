...
describe("InboxReader attachments", () => {
  it("links only http(s) attachment urls, in a new tab without opener or referrer", async () => {
    ...
    expect(screen.getByText("invoice.html")).toHaveTextContent("link unavailable")
    expect(screen.getByText("raw.bin")).toHaveTextContent("link unavailable")
    expect(screen.getAllByText("link unavailable")).toHaveLength(2)
